...
const TEXT_ENCODER_PIPELINE = ["data", "hexadecimal", "binary", "ANTC"];
...
function prepareEncodePayload(input, requestedMode) {
  ...
  const resolvedMode = resolveInputMode(requestedMode);
  const actualMode =
    resolvedMode === INPUT_MODE_AUTO
      ? looksLikeBinary(source)
        ? INPUT_MODE_RAW
        : INPUT_MODE_TEXT_HEX_BINARY
      : resolvedMode;

  if (actualMode === INPUT_MODE_RAW) {
    return {
      payload: source,
      metadata: {
  ...
  return {
    payload: transformed,
    metadata: {
      input_mode: INPUT_MODE_TEXT_HEX_BINARY,
      source_size: source.length,
      source_format: "text",
      preprocessing: "data-to-hex-to-binary",
      preprocessing_pipeline: TEXT_ENCODER_PIPELINE
    }
  };
}
...
function buildPreview(parsed, maxPreviewPixels = MAX_PREVIEW_PIXELS) {
  ...
  if (parsed.pixelCount !== width * height) {
    return null;
  }
  ...
  const displayPixels =
    displayPixelCount === parsed.pixelCount
      ? parsed.pixelData
      : Buffer.concat([parsed.pixelData, Buffer.alloc((displayPixelCount - parsed.pixelCount) * 3)]);
