...
const { decodeFile, encodeFile, inspectFile, preflightEncodeFile } = require("./file-ops");

function usage() {
  return [
    "Usage:",
    "  atl_codex encode <input> <output> [--compression auto|none|zlib-fast|zlib|zlib-max|gzip-fast|gzip|gzip-max|brotli-fast|brotli|brotli-max] [--auto-strategy smart|speed|balanced|max|exhaustive] [--transform none|auto|bwt|mtf|bitplane|delta|rle|delta-rle] [--combo combo-none|delta+rle|brotli+delta|zlib+rle] [--input-mode auto|raw] [--width N] [--height N] [--metadata-file path]",
    "  atl_codex preflight <input> [--compression auto|none|zlib-fast|zlib|zlib-max|gzip-fast|gzip|gzip-max|brotli-fast|brotli|brotli-max] [--auto-strategy smart|speed|balanced|max|exhaustive] [--transform none|auto|bwt|mtf|bitplane|delta|rle|delta-rle] [--combo combo-none|delta+rle|brotli+delta|zlib+rle] [--input-mode auto|raw] [--width N] [--height N] [--metadata-file path] [--sample-fraction 0.03]",
...
function parseArguments(argv) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg === "--compression") {
      ...
      options.comboCompression = value;
      ...
      options.sampleFraction = Number.parseFloat(value);
    } else {
...
function runEncode(args) {
  ...
  const result = encodeFile({
    ...
    comboCompression: options.comboCompression,
...
function runPreflight(args) {
  ...
  if (positionals.length !== 1) {
    throw new Error("preflight requires <input>.");
  }

  const [inputPath] = positionals;
  const result = preflightEncodeFile({
    inputPath,
    compression: options.compression,
    comboCompression: options.comboCompression,
    autoCompressionStrategy: options.autoCompressionStrategy,
    ...
    height: options.height,
    metadataPath: options.metadataFile,
    sampleFraction: options.sampleFraction
  });
